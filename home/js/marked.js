/*!
 * http://www.bimwook.com/
 *
 * Copyright (c) 2016 Yangbo
 *
 * Date: 2016-01-06 16:00:00 +0800
 * Revision: 1
 */
  ...
  },  
  {
    ...
    execute: function(data){
      while(true){
        var mcs = (data||"").match(this.regex);
        if(!mcs) break;
        var s = '<a target="_blank" href="' + marked.encode(mcs[2].replace(/&amp;/ig, "&").replace(/(<i>)|(<\/i>)/ig, "_")) + '" title="' + marked.encode((mcs[3]||"").replace(/(<i>)|(<\/i>)/ig, "_")) + '">' + marked.encode(mcs[1]) + '</a>';        
        data = data.replace(this.regex, s);
      }
      return data;      
    }
...
marked.parse = function(data){
  ...
  for(var i=0; i<lines.length; i++){
    ...
    else for(var rr=0; rr<this.items.length; rr++){
